...
STOP_CAPTURE_HOOK_FILENAME = "stop_capture_agent_output_hook.py"
...
def _main():
    try:
        sys.path.insert(0, PACKAGE_DIRECTORY_ABSOLUTE_PATH)
        from source_of_truth.stop_hook_capture_agent_output import _main as inner_main
    except Exception:
        _emit_empty_and_exit_silently(); return
    try:
        sys.exit(inner_main())
...
def write_wrapper_scripts(install_dir):
    ...
    with open(stop_capture_hook_path, "w") as f:
        f.write(STOP_CAPTURE_HOOK_TEMPLATE.format(package_dir=install_dir))
...
def ensure_settings_json_hook_entries(install_dir, settings_path, python_launcher):
    ...
    stop_capture_command = f'{python_launcher} "{stop_capture_hook_path}"'
    ...
    removed_stop = remove_stale_source_of_truth_hook_entries(
        settings_data, "Stop", install_dir
    )
    if removed_user + removed_post + removed_stop:
        print(f"  removed {removed_user} stale UserPromptSubmit + "
              f"{removed_post} stale PostToolUse + {removed_stop} stale Stop "
              f"entries pointing elsewhere")
    ...
    # Stop hook: capture the turn's agent output as the next prompt's pre-text.
    existing_stop_commands_in_settings = [
        hook_item.get("command", "")
        for matcher_entry in settings_data.get("hooks", {}).get("Stop", [])
        for hook_item in matcher_entry.get("hooks", [])
    ]
    if stop_capture_command not in existing_stop_commands_in_settings:
        append_hook_entry(settings_data, "Stop", "*", stop_capture_command)
        print(f"  appended Stop(matcher=*) agent-output capture: {stop_capture_command}")
        settings_was_modified = True
    else:
        print("  Stop (agent-output capture) entry already present -- no change")
...
def main():
    ...
    print(f"      * UserPromptSubmit (raw input capture)")
    print(f"      * PostToolUse:Bash (SoT messages-to-user)")
    print(f"      * PostToolUse:* (what-is-session-id sentinel hook)")
    print(f"      * Stop (capture agent output as next prompt's pre-text)")
