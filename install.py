...
PostToolUse hook. Like every other source-of-truth hook it SELF-GATES on
project membership: it does nothing unless the current session is enrolled in a
source-of-truth project, so unsubscribed sessions are never affected. For an
enrolled session it also only speaks when the just-completed tool call's
tool_input contains the sentinel string {sentinel_string!r}; then it surfaces the
current session_id to both the user (systemMessage) and the agent
(additionalContext). On any error it silently emits {{}}.
"""
import json
import sys
...
def _main():
    ...
    current_session_id = hook_event_payload.get("session_id")
    if not current_session_id:
        _emit_empty_and_exit_silently(); return

    # Membership gate: same as every other SoT hook -- do nothing for a session
    # that is not enrolled in a source-of-truth project.
    try:
        sys.path.insert(0, PACKAGE_DIRECTORY_ABSOLUTE_PATH)
        from source_of_truth.project_identifier_resolver import resolve_project_id_for_session
    except Exception:
        _emit_empty_and_exit_silently(); return
    if resolve_project_id_for_session(current_session_id) is None:
        _emit_empty_and_exit_silently(); return
