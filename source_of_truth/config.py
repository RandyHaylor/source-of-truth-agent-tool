...
INTERACTION_TIME_AGENT_GUIDANCE: str = (
    "[SoT]\n"
    "Before add: search/read existing tree. Repeat=no-op (or modify_reference). "
    "Supersede=remove stale + add new in same change-set. New topic=add. "
    "Never leave conflicting nodes side-by-side.\n"
    "Capture: every definitive user reply (explicit OR pick from your A/B/C) -> add op "
    "ref'ing raw_input_id (pre_text carries the question). Prefer isolated yes/no or A/B/C "
    "questions; brief answers cite cleanly. Never paraphrase into user's mouth.\n"
    "Op rules:\n"
    "  parent_id required: '0'=top-level only if no fitting parent. Prefer adding under groups.\n"
    "  <short_neutral_title>: 1-5 word noun phrase TOPIC, 1-50 chars. NOT a sentence/rule.\n"
    "    good: \"vendor placement\", \"phrase character voice\"\n"
    "    bad:  \"Phrases must be in-character\" (sentence), \"vendor every other floor\" (spec detail)\n"
    "    Reviewer auto-amends overreaches via amended_short_title; persisted title shows in\n"
    "    submit response's assigned_node_ids.\n"
    "  char_range: forbidden if submission<=500 chars; allowed-but-optional if >500.\n"
    "Batch related ops in ONE submit-change-set call.\n"
    "On result: RELAY message_for_raw_input_sender VERBATIM to user, then proceed (approve) or "
    "adjust+resubmit / ask clarifying Q (reject).\n"
)
