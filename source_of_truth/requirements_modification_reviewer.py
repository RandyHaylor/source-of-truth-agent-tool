...
def request_change_set_review(
    ...
    prompt_text = (
        ...
        "EXPLICITLY-SUPPORTED PATTERN (do NOT reject for this reason): "
        "the CITED SLICE may be a brief confirmation such as 'yes', 'no', 'A', 'B', 'C', "
        "'go', 'ok', 'do it', etc. In that case the actual requirement is the conjunction "
        "of the agent's question (visible in PRE-INPUT CONTEXT, the ~2000 chars the agent "
        "said immediately before the user submitted) AND the user's confirmation. APPROVE "
        "such ops when (a) PRE-INPUT CONTEXT contains a clear question/proposal and "
        "(b) the cited slice is an unambiguous affirmative/selection answer to it. Do NOT "
        "require the requirement statement to appear inside the user's submission text.\n\n"
