...
- **MUST-CAPTURE instructions.** Every explicit user instruction, decision, or answer (e.g. "do X", "use library Y", "make it blue", "yes") is filed under `pending-instructions` before the agent acts, then reparented to `completed-instructions` or `deprecated-instructions` as it's carried out or dropped — so the live set of standing instructions is always visible in the tree.
