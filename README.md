...
- **Default-scaffolded subject groups** (`resources`, `user-interaction-preferences`, `technical-requirements`, `current-project-documentation`, plus the instruction-lifecycle groups `pending-instructions`, `completed-instructions`, `deprecated-instructions`) give every new project a consistent logical skeleton to file into from turn one.
- **MUST-CAPTURE instructions.** Every explicit user instruction, decision, or answer (e.g. "do X", "use library Y", "make it blue", "yes") is filed under `pending-instructions` before the agent acts, then reparented to `completed-instructions` or `deprecated-instructions` as it's carried out or dropped — so the live set of standing instructions is always visible in the tree. The agent **manages** the tree (it doesn't ask permission to add/reparent); it simply **notifies** the user when it moves a node to completed/deprecated. Using the ledger is required — it's the only place guaranteed verbatim user quotes live as requirements.
