...
**MAINTAIN — prune conflicts after every addition.** Capture is not only additive. After you add a requirement, **review the existing tree for conflicts**: if the new prompt supersedes, changes, or contradicts an existing requirement node, **reparent that now-stale node to `deprecated-instructions`** (or `remove` it) so the tree holds **only current, non-conflicting truth**. These structural ops auto-approve. A tree that only ever grows drifts from reality; keeping it pruned is part of the per-turn duty.
