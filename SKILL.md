...
**Capture is judgement, not a checklist — the tree is a living project doc.** Its purpose is to reliably attach the user's own quotes and confirmations to a node tree that documents both their **requests** and the **project's requirements/information**. Capture anything that carries durable meaning — a settled instruction, decision, request, answer, or a fact about the project — and store it *before* you act on it. Skip pure transient chatter (a passing question, thinking-aloud, brainstorming still taking shape), but **don't draw hard lines or agonize over categories**: the failure mode to avoid is a tree that misses real decisions, not one that occasionally holds an extra confirmation. When unsure whether something is settled, confirm it (see below) rather than skip it. Examples clearly worth capturing:
...
**MAINTAIN — prune conflicts after every addition.** Capture is not only additive. After you add a requirement, **review the existing tree for conflicts**: if the new prompt supersedes, changes, or contradicts an existing requirement node, **reparent that now-stale node to `deprecated-instructions`** (or `remove` it) so the tree holds **only current, non-conflicting truth**. These structural ops auto-approve. **Prioritize keeping the tree pruned** — promptly retire deprecated/superseded entries and don't let stale ones linger; a tree that only ever grows drifts from reality, so pruning is a first-class part of the per-turn duty, not an afterthought.
